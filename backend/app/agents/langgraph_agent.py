...
def _fallback_correction(message: str, current_fields: ExtractedFields) -> dict:
    ...
    patterns = {
        "batch_number": r"batch\s*(?:/\s*lot)?\s*(?:number|no\.?)?\s*(?:is|:)?\s*([A-Za-z0-9\- ]{4,20})(?:\s*and\b|\.|$)",
        "affected_quantity": r"(?:affected\s*quantity|quantity)\s*(?:is|:)?\s*([\d,]+\s?[A-Za-z0-9()\-\s]{2,30})",
    ...
    for field, pattern in patterns.items():
        ...
        if m:
            ...
            # Trim a keyword from a following field that got glued on with no
            # separating space or punctuation (e.g. "...50 kg (2 HDPE Drum)quantity is...").
            value = re.sub(
                r"\s*\b(and|affected|quantity|manufacturing|expiry|customer)\b\s*$",
