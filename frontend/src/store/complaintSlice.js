...
export const PARSE_KIND = { TEXT: 'text', FILE: 'file' }
...
const complaintSlice = createSlice({
  ...
  initialState: {
    ...
    sourceFilename: '',
    parsing: false,
    parsingKind: null, // 'text' | 'file' | null
    committing: false,
    error: null,
    committedRef: null,
  },
  ...
  extraReducers: (builder) => {
    builder
      .addCase(parseComplaint.pending, (state, action) => {
        state.parsing = true
        state.parsingKind = action.meta.arg?.file ? 'file' : 'text'
        state.error = null
      })
      .addCase(parseComplaint.fulfilled, (state, action) => {
        state.parsing = false
        state.parsingKind = null
