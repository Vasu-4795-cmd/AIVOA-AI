...
const OCR_STEPS = [
  ...
  'Mapping fields to QMS schema...',
  'Running initial risk assessment...',
]

function FileParsingProgress() {
  ...
  useEffect(() => {
    ...
    const pctTimer = setInterval(() => {
      setPct((p) => (p < 92 ? p + Math.random() * 10 : p))
    }, 350)
    return () => {
  ...
  return (
    <div className="bubble assistant progress-bubble">
      <div className="progress-label">{OCR_STEPS[stepIndex]}</div>
      <div className="progress-track">
        <div className="progress-fill" style={{ width: `${Math.min(pct, 92)}%` }} />
      </div>
    </div>
  )
}
...
export default function CopilotPanel() {
  ...
  const { parsing, parsingKind, status } = useSelector((s) => s.complaint)
